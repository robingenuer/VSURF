...
test_that("Variable importance",{
  ...
  expect_equal(orange.vsurf$imp.sd.dec,
               ...
               tolerance = 1e-3)
  expect_identical(orange.vsurf$imp.mean.dec.ind, c(2L, 1L, 5L, 8L, 6L, 3L, 9L, 7L, 4L))
})

test_that("OOB erros of nested models", {
  expect_equal(orange.vsurf$err.interp, c(719.6904, 464.9525, 784.7824),
               tolerance = 1e-3)
  expect_equal(orange.vsurf$err.pred, 724.8328, tolerance = 1e-3)
})

test_that("Thresholds for the 3 steps", {
  expect_equal(min(orange.vsurf$pred.pruned.tree), 42.07285,
               tolerance = 1e-3)
  expect_equal(orange.vsurf$sd.min, 28.52897, tolerance = 1e-3)
  expect_equal(orange.vsurf$mean.jump, 319.8299, tolerance = 1e-3)
})